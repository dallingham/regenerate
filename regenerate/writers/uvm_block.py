...
class UVMBlockRegisters(WriterBase):
    ...
    def write_toplevel_block_new(self, of):
        func_def = ("", '   function new(string name = "{0}_reg_block");',
    ...
    def write_toplevel_block_build(self, of):
        ...
        for key in map2grp:
            ...
            of.write(
                '         {0} = create_map("{0}", \'h{1:x}, {2:d}, {3});\n'.format(
                    map_name, map2base[key],
    ...
    def connect_submaps(self, of, grp_data, data_name, name):
        ...
        if grp_data.repeat <= 1:
            ...
            of.write('         if (!{0}) begin\n'.format(disable))
            of.write(
                "            {0}.add_submap({1}[i].{0}, 'h{2:x} + (i * 'h{3:x}));\n".format(
    ...
    def build_and_configure_group_array(self, of, group, map2grp):
        ...
        of.write('      foreach ({0}[i]) begin\n'.format(name))
        of.write(
            '         {0}[i] = {1}::type_id::create($sformatf("{0}[%0d]", i));\n'.format(
                name, cls))
        if group.hdl:
            of.write(
                '         {0}[i].configure(this, $sformatf("{1}", i));\n'.format(
                    name, group.hdl))
        else:
    ...
    def write_toplevel_block(self, of):
        ...
        header = ("/*", " * Top level register block", " */", "",
    ...
    def write_group_block(self, group, of, in_maps):
        ...
        for item in in_maps:
            ...
            of.write(
                '         {0}_map = create_map("{0}_map", 0, {1}, {2});\n'.format(
        ...
        for group_entry in group.register_sets:
            ...
            if group_entry.repeat > 1 or group_entry.array:
                of.write('      for(int i = 0; i < {0}; i++) begin\n'.format(
                    group_entry.repeat))
                of.write(
                    '         {0}[i] = {1}_{2}_reg_blk::type_id::create($sformatf("{0}[%0d]", i));\n'.format(
                        inst, sname, name))
                if group_entry.hdl:
                    of.write(
                        '         {0}[i].configure(this, $sformatf("{1}", i));\n'.format(
                ...
                for item in in_maps:
                    ...
                    of.write(
                        "            {0}.add_submap({1}[i].{0}, 'h{2:x} + (i * 'h{3:x}));\n".format(
                            mname, inst, group_entry.offset,
                            group_entry.repeat_offset))
                    of.write("         end\n")
                    if group_entry.no_uvm:
                        of.write(
                            '        uvm_resource_db#(bit)::set({{"REG::",{0}[i].get_full_name(),".*"}},\n'.format(
                                inst))
                        of.write(
                            '                                    "NO_REG_TESTS", 1, this);\n')
                of.write('      end\n')
                ...
                of.write(
                    '      {0} = {1}_{2}_reg_blk::type_id::create("{0}");\n'.format(
                        inst, sname, name))
                of.write('      {0}.configure(this, "{1}");\n'.format(
                    inst, group_entry.hdl))
                for item in in_maps:
                    of.write(
                        "      {0}.disable_{1}_map = disable_{1}_map;\n".format(
                            inst, item))
                of.write("      {0}.build();\n".format(inst))
                for item in in_maps:
                    of.write("      if (!disable_{0}_map) begin\n".format(item))
                    of.write(
                        "         {0}_map.add_submap({1}.{0}_map, 'h{2:x});\n".format(
                            item, inst, group_entry.offset))
                    of.write("      end\n")
                if group_entry.no_uvm:
                    of.write(
                        '      uvm_resource_db#(bit)::set({{"REG::",{0}.get_full_name(),".*"}},'.format(
                            inst))
    ...
    def write_dbase_block(self, dbase, of, group, in_maps):
        ...
        of.write(
            '        {0}_access_cg = {0}_reg_access_wrapper::type_id::create("{0}_access_cg");\n'.format(
                sname))
        ...
        for reg in dbase.get_all_registers():
            ...
            of.write(
                '      {0} = {1}::type_id::create("{0}", , get_full_name());\n'.format(
                    token, rname, token))
            for field in reg.get_bit_fields():
                if field.reset_type == BitField.RESET_PARAMETER:
                    if field.reset_parameter:
            ...
            if reg.ram_size == 0:
                ...
                if not reg.do_not_generate_code:
                    for field in reg.get_bit_fields():
                        of.write(
                            '      %s.add_hdl_path_slice("r%02x_%s", %d, %d );\n' %
                            (token, reg.address, self._fix_name(field), field.
                             lsb, field.width))
        of.write("\n")
        ...
        of.write(
            '          {0}_access_cg.sample(offset, is_read);\n'.format(sname))
        of.write('       end\n')
    ...
    def disable_access_tests(self, dbase, of):
        for reg in dbase.get_all_registers():
            ...
            of.write(
                'get_full_name(), ".{0}"}}, "NO_{1}_ACCESS_TEST", 1);\n'.format(
    ...
    def write_memory(self, reg, dbase, of):
        ...
        of.write(
            '       super.new(name, {0}, {1}, "{2}", UVM_NO_COVERAGE);\n'.format(
    ...
    def write_db_coverage(self, of, dbase):
        ...
        of.write("\n\n")
        of.write(
            "class {0}_reg_access_wrapper extends uvm_object;\n".format(base))
        of.write(
        ...
        of.write(
            '   function new(string name = "{0}_reg_access_wrapper");\n'.format(
                base))
